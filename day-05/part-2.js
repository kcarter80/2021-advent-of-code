...
readInterface.on('line', function(line) {
    ...
    lines.forEach(coordinates => {
            ...
            else if (coordinates.y1 == coordinates.y2 && coordinates.x1 > coordinates.x2) {
            ...
            // not vertical or horizontal, so must be diagonal
            else {
                let xStart = coordinates.x1
                ...
                const yStart = Math.min(coordinates.y1,coordinates.y2)
                const amount = Math.abs(coordinates.x1 - coordinates.x2)
                for (let i = 0; i <= amount; i++) {
                    if (right) diagram[yStart + i][xStart + i] += 1
                    else diagram[yStart + i][xStart - i] += 1
                }
            }
    })

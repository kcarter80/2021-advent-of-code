// 20 minutes to code, ~20 minutes to run
    ...
    for(let i = 1; i < scanners.length; i++) {
        ...
        for(let ii = 0; ii < rotatedScanners.length; ii++) {
            ...
            if (results.maxMatches >= 12) {
                ...
                scannerLocations.push([results.incrementX,results.incrementY,results.incrementZ])
                ...
                scanners.splice(i,1)
                i = 0
                console.log('scanners left',scanners.length,'beacons',scanners[0].length)
                break
            }
        }
    }
    console.log(scannerLocations)
    let maxManhattan = null
    for(let i = 0; i < scannerLocations.length; i++) {
        for(let ii = 0; ii < scannerLocations.length; ii++) {
            if (i != ii) {
                let thisManhattan = Math.abs(scannerLocations[i][0] - scannerLocations[ii][0]) + 
                    Math.abs(scannerLocations[i][1] - scannerLocations[ii][1]) +
                    Math.abs(scannerLocations[i][2] - scannerLocations[ii][2])
                if (maxManhattan == null || maxManhattan < thisManhattan)
                    maxManhattan = thisManhattan
            }
        }
    }

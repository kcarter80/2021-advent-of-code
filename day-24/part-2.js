...
readInterface.on('line', function(line) {
    ...
    for(let input = 0; input < 14; input++) {
        ...
        if (input == 1) {
            results = {
                ...
                //'6,1,7,7': 6,
                //'7,1,8,8': 7,
                '8,1,9,9': 8, // 84191521311611
                ////'9,1,10,10': 9  //94191521311612
            }
        }     
    ...
    for (let i = 0; i < resultKeys.length; i++) {
        if (resultKeys[i].split(',')[3] == 0) {
            if (!minInput) minInput = results[resultKeys[i]]
            else {
                if (results[resultKeys[i]] < minInput) minInput = results[resultKeys[i]]
            }
        }
    }

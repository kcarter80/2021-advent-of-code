...
class pair {
    ...
    reduce() {
        //console.log('evaluating for reduction',this.displayString())
        let reduced = false
        let lmfdc = this.findLeftMostFourDeepChild()
        if (lmfdc) {
            console.log('exploding',lmfdc.displayString())
            this.explode(lmfdc)
            reduced = true
        } else {
            reduced = this.splitLeftMostEligible()
            if(reduced) console.log('split')
        }
...
readInterface.on('line', function(line) {
    let currentArray = eval(line)
    if (firstLine) {
        currentSum = currentArray
        firstLine = false
    } else {
        currentSum = [currentSum,currentArray]
    }
    sn = new pair(null,currentSum[0],currentSum[1])
    sn.reduce()
    currentSum = eval(sn.displayString())
}).on('close', function() {
    console.log('end')
    console.log(sn.displayString())
    console.log(sn.magnitude())
})
